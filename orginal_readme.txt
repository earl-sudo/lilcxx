...
--------------------------------
 ...
 threads even if different threads use different LilInterp_Ptr objects.  Also it
...
4.1. Initialize LIL
 ...
 others and have its own definitions.  A LIL runtime is stored in the LilInterp_Ptr
 C type that basically specifies an object.  To construct a LilInterp_Ptr object
 ...
 has the following signature:
     ...
     void lil_callback(LilInterp_Ptr lil,
                       int cb,
 ...
 following code
     ...
     void handl_exit(LilInterp_Ptr lil, Lil_value_Ptr arg)
     {
         ...
     }
...
4.2. Execute LIL code
 ...
 following signature:
     ...
     Lil_value_Ptr lil_parse(LilInterp_Ptr lil,
                           const char* code,
                           size_t codelen,
                           int funclevel)
 ...
 parse code which is stored in a LIL value.  The signature is
     ...
     Lil_value_Ptr lil_parse_value(LilInterp_Ptr lil,
                                 Lil_value_Ptr val,
                                 int funclevel)
 ...
 has the following signature:
     ...
     Lil_value_Ptr lil_call(LilInterp_Ptr lil,
                          const char* funcname,
                          size_t argc,
                          Lil_value_Ptr* argv)
 ...
 lil_break_run function with the following signature:
     ...
     int lil_break_run(LilInterp_Ptr lil,
                       int dobreak)
...
4.3. Raise an error and obtain information about errors
 ...
 using the lil_set_error() function which has the following signature
     ...
     void lil_set_error(LilInterp_Ptr lil,
                        const char* msg)
 ...
 message available.  The signature of the function is
     ...
     int lil_error(LilInterp_Ptr lil,
                   const char** msg,
                   size_t* pos)
...
4.4. Convert values between C and LIL
     --------------------------------
   LIL uses the Lil_value_Ptr type for its own values.  To convert from C to
 Lil_value_Ptr you use the following functions:
 
     Lil_value_Ptr lil_alloc_string(const char* str)
     Lil_value_Ptr lil_alloc_double(double num)
     Lil_value_Ptr lil_alloc_integer(lilint_t num)
 ...
 (see next).
   ...
   To convert a Lil_value_Ptr to a C type use one of the following functions:

     const char* lil_to_string(Lil_value_Ptr val)
     double lil_to_double(Lil_value_Ptr val)
     lilint_t lil_to_integer(Lil_value_Ptr val)
     int lil_to_boolean(Lil_value_Ptr val)
 ...
 at the end of existing values using one of the following functions:
     ...
     Lil_value_Ptr lil_clone_value(Lil_value_Ptr src)
     int lil_append_char(Lil_value_Ptr val, char ch)
     int lil_append_string(Lil_value_Ptr val, const char* s)
     int lil_append_val(Lil_value_Ptr val, Lil_value_Ptr v)

   Using the following functions you can manipulate LIL lists:

     Lil_list_Ptr lil_alloc_list(void)
     ...
     Lil_value_Ptr lil_list_get(Lil_list_Ptr list, size_t index)
     Lil_value_Ptr lil_list_to_value(Lil_list_Ptr list, int do_escape)

 the last function will convert a Lil_list_Ptr to a LIL value.  Unless you are
...
4.5. Register native functions
 ...
 function has the following signature:
     ...
     int lil_register(LilInterp_Ptr lil,
                      const char* name,
 ...
 native function must have the following signature:
     ...
     Lil_value_Ptr lil_func_proc(LilInterp_Ptr lil,
                               size_t argc,
                               Lil_value_Ptr* argv)
 
 where lil is the lil object that issued the function, argc is the number
 of arguments used and argv is an array of "argc" Lil_value_Ptr elements with
 the arguments passed to the function.  The function must return a newly
 allocated Lil_value_Ptr object or NULL if the function returns nothing (or
 returns an empty string).
 
   An example:
   
     static Lil_value_Ptr fnc_hi(LilInterp_Ptr lil, size_t argc, Lil_value_Ptr* argv)
     {
         printf("Hi!\name");
         return NULL;
     }
...
4.6. Other LIL library functions
   ...
   Some other library functions are
     ...
     Lil_var_Ptr lil_set_var(LilInterp_Ptr lil,
                           const char* name,
                           Lil_value_Ptr val,
                           int local)
 ...
 good candidate).
     ...
     Lil_value_Ptr lil_get_var(LilInterp_Ptr lil,
                             const char* name)
     
 which can be used to return the value of a variable.  Alternatively
     
     Lil_value_Ptr lil_get_var_or(LilInterp_Ptr lil,
                                const char* name,
                                Lil_value_Ptr defvalue)
 ...
 function called from a script function).
     ...
     Lil_value_Ptr lil_eval_expr(LilInterp_Ptr lil,
                               Lil_value_Ptr code)

 which can be used to evaluate an expression like the "expr" function.
 
     Lil_value_Ptr lil_unused_name(LilInterp_Ptr lil,
                                 const char* part)
 ...
 associate a pointer to a LilInterp_Ptr object and retrieve it:
 
     void lil_set_data(LilInterp_Ptr lil,
                       void* data)
  
  which associates "data" with the given lil object and
  
     void* lil_get_data(LilInterp_Ptr lil)
 ...
 lil_embedded like:
     ...
     char* lil_embedded(LilInterp_Ptr lil,
                        const char* code,
                        unsigned int flags)
 ...
 the internal buffer used in lil_embedded) using the lil_write function:
     ...
     void lil_write(LilInterp_Ptr lil, const char* msg)
...
4.7. LIL callback summary
 ...
 lil_callback() function:
     ...
     LIL_CALLBACK_EXIT
       ...
       signature: void (LilInterp_Ptr lil, Lil_value_Ptr arg)
       called:    when the LIL "exit" function is called
       
     LIL_CALLBACK_WRITE:
       callback:  lil_write_callback_proc_t
       signature: void (LilInterp_Ptr lil, const char* msg)
     ...
     LIL_CALLBACK_READ:
       ...
       signature: char* (LilInterp_Ptr lil, const char* name)
     ...
     LIL_CALLBACK_STORE:
       ...
       signature: void (LilInterp_Ptr lil, const char* name, const char* data)
       called:    to store "data" in the file "name" as its contents

     LIL_CALLBACK_ERROR:
       callback:  lil_error_callback_proc_t
       signature: void (LilInterp_Ptr lil, size_t pos, const char* msg)
       called:    when an error occurs and is not handled
     
     LIL_CALLBACK_SETVAR:
       callback:  lil_setvar_callback_proc_t
       signature: int (LilInterp_Ptr lil, const char* name, Lil_value_Ptr* value)
     ...
     LIL_CALLBACK_EMBEDDEDFILTER
       ...
       signature: const char* (LilInterp_Ptr lil, const char* msg)
...
4.8. Using LIL as a DLL
 ...
 couple of extra steps to be performed:
     ...
     * The LILCALLBACK macro must be used for callbacks.  For example this
           ...
           static Lil_value_Ptr fnc_hi(LilInterp_Ptr lil,
                                     size_t argc,
                                     Lil_value_Ptr* argv)
       
       must be changed to this

           static LILCALLBACK Lil_value_Ptr fnc_hi(LilInterp_Ptr lil,
                                                 size_t argc,
                                                 Lil_value_Ptr* argv)
